...
export const Section: React.FC<{ id?: string, className?: string, children?: React.ReactNode }> = ({ id, className = "bg-pure-white text-deep-black", children }) => (
    <section id={id} className={`py-[80px] md:py-[120px] relative ${className}`}>
        {children}
    </section>
);
...
export const Button: React.FC<{ children?: React.ReactNode; variant?: "primary" | "secondary" | "dark" | "outline"; className?: string; onClick?: () => void }> = ({ children, variant = "primary", className = "", onClick }) => {
    const baseStyles = "inline-flex items-center justify-center font-bold font-space uppercase tracking-wide py-4 px-8 rounded-sm transition-all duration-300 transform hover:-translate-y-1 text-sm md:text-base cursor-pointer relative overflow-hidden group active:scale-95";
    const variants = {
        primary: "bg-neon-green text-deep-black border-2 border-neon-green hover:shadow-[0_0_25px_rgba(27,252,79,0.4)] hover:text-deep-black",
             ...
             <div className="w-full lg:w-1/3 flex flex-col justify-center gap-4 p-4 lg:p-8">
                 {slides.map((slide, idx) => (
                     <div 
                        ...
                        className={`p-6 border-l-2 cursor-pointer transition-all duration-300 group ${
                            activeIndex === idx 
                ...
                <div className="flex flex-col gap-2">
                    {tabs.map((tab, idx) => (
                        <div 
                            ...
                            className={`group relative p-3 md:p-4 cursor-pointer transition-all duration-300 border-l-2 ${
                                activeTab === idx 
                    ...
                    {tabs.map((tab, idx) => (
                         <div 
                            ...
                            className={`absolute inset-0 transition-all duration-500 ease-in-out ${
                                activeTab === idx ? 'opacity-100 scale-100 z-10' : 'opacity-0 scale-95 z-0'
                            }`}
                        >
                            <img 
                                src={tab.img} 
